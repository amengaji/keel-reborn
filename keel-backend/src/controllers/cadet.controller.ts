...
export const getCadets = async (req: Request, res: Response) => {
  try {
    ...
    // 1. FILTER BY COMPANY (Multi-Tenancy)
    if (currentUser && currentUser.role !== 'SUPER_ADMIN' && currentUser.company_id) {
        whereClause.company_id = currentUser.company_id;
    }

    // 2. MASTER / CTO VESSEL FILTER (Crucial for Context Isolation)
    // If the requester is assigned to a vessel (Master/CTO), restrict search to that vessel
    if ((isMaster || isCTODeck) && userVesselId) {
        whereClause.vessel_id = userVesselId;
    }
    ...
    const cadets = await User.findAll({
      ...
      include: [
        ...
        },
        {
          ...
          as: 'assignments',
          where: { status: 'ACTIVE' },
          required: (isMaster || isCTODeck), // If Master/CTO, they only care about ACTIVE onboard cadets
          include: [
            { 
              model: Vessel, 
              as: 'vessel',
    ...
    // ----------------------------------------------------
    // CTO DECK POST-FILTER (Rank-based logic)
    // ----------------------------------------------------
    const filteredCadets = isCTODeck
      ? cadets.filter((c: any) => {
    ...
    const formattedCadets = filteredCadets.map((c: any) => {
      ...
      const vesselName = activeAssignment?.vessel?.name || plainCadet.vessel?.name || null;
...
export const deleteCadet = async (req: Request, res: Response) => {
  try {
    ...
    await Assignment.destroy({ where: { user_id: id } }); 
...
/**
 * GET SINGLE CADET BY ID
 */
export const getCadetById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    
    // Find the user and include Vessel/Role to satisfy the UI requirements
    const cadet = await User.findByPk(id, {
      attributes: { exclude: ['password_hash'] },
      include: [
        { model: Role, as: 'role' },
        { 
    ...
    // Return the raw object (frontend cadetService handles the rest)
    res.json(cadet);
  } catch (error: any) {
    console.error("GET CADET BY ID ERROR:", error);
    res.status(500).json({ 
      message: "Error retrieving trainee profile", 
      error: error.message 
    });
  }
};