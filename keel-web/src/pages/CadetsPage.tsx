...
const CadetsPage: React.FC = () => {
  ...
  const handleAddCadet = (data: any) => {
    ...
    saveCadet(newCadet); // Save to Service
    setCadets(getCadets());
  ...
  const handleImportCadets = (importedData: any[]) => {
    ...
    const currentCadets = getCadets();
    const updatedList = [...currentCadets, ...newCadets];
    saveAllCadets(updatedList); // Save All
    setCadets(updatedList);
    toast.success(`${newCadets.length} cadet profiles imported.`);
  };

  const handleDelete = (id: number) => {
    const updated = cadets.filter(c => c.id !== id);
    saveAllCadets(updated); // Update Service
    setCadets(updated);
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden min-h-100">
        <div className="p-4 border-b border-border bg-muted/30 flex justify-between items-center">
          <div className="relative w-72">
            ...
            <input type="text" placeholder="Search by Name or Email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full bg-background pl-9 pr-4 py-2 rounded-md border border-input text-sm focus:ring-1 focus:ring-primary outline-none transition-all placeholder:text-muted-foreground" />
          </div>
              ...
              {filteredCadets.map((cadet) => (
                <tr key={cadet.id} className="hover:bg-muted/30 transition-colors group">
                  <td className="px-6 py-4 font-medium text-foreground flex items-center space-x-3">
                    <div className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-primary font-bold text-xs shrink-0">{cadet.name.charAt(0)}</div>
                    <div><p className="font-semibold">{cadet.name}</p><p className="text-[10px] text-muted-foreground">{cadet.email}</p></div>
                  </td>
                  <td className="px-6 py-4 text-muted-foreground">{cadet.rank}<div className="text-[10px] opacity-70">{cadet.nationality}</div></td>
                  ...
                  <td className="px-6 py-4">
                    ...
                    }`}>{cadet.status.toUpperCase()}</span>
                  </td>
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    <button onClick={() => handleDelete(cadet.id)} className="text-muted-foreground hover:text-destructive transition-colors p-1"><Trash2 size={16} /></button>
                    <button className="text-muted-foreground hover:text-primary transition-colors p-1"><MoreVertical size={18} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <AddCadetModal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} onSave={handleAddCadet} />
      <ImportCadetModal isOpen={isImportOpen} onClose={() => setIsImportOpen(false)} onImport={handleImportCadets} />
    </div>
  );
};
