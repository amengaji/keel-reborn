...
import { 
  ...
  getSyllabus, calculateProgressStats, getSettings
      ...
      {data.map((slice, i) => {
        ...
        return (
          <svg key={i} viewBox="0 0 32 32" className="absolute inset-0 w-full h-full">
            <circle cx="16" cy="16" r="16" fill="transparent" stroke={slice.color} strokeWidth="32" strokeDasharray={dashArray} strokeDashoffset={dashOffset} />
          </svg>
        );
      })}
...
const Dashboard: React.FC = () => {
  ...
  const QuickActionBtn = ({ icon, label, onClick, colorClass }: any) => (
    ...
    >
      <div className={`p-3 rounded-full ${colorClass} group-hover:scale-110 transition-transform`}>{icon}</div>
      ...
      <div className="space-y-6 animate-in fade-in duration-500 pb-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-sm flex items-center gap-2">
              <Server size={14} /> Platform Overview & System Health
            </p>
          </div>
          <div className="flex gap-3">
             <button onClick={() => navigate('/companies')} className="bg-primary hover:bg-primary/90 text-primary-foreground px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm text-sm font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-card p-5 rounded-xl border border-border shadow-sm relative overflow-hidden">
             ...
             <div className="mt-4 flex gap-2">
                <span className="text-xs bg-green-500/10 text-green-600 px-2 py-1 rounded font-medium">8 Active</span>
                <span className="text-xs bg-yellow-500/10 text-yellow-600 px-2 py-1 rounded font-medium">4 Trial</span>
             </div>
          </div>
          <div className="bg-card p-5 rounded-xl border border-border shadow-sm relative overflow-hidden">
             ...
             <h3 className="text-3xl font-bold text-foreground mt-2">{metrics.totalCadets + 50}</h3>
             <p className="text-xs text-muted-foreground mt-4">Across all tenants</p>
          </div>
          <div className="bg-card p-5 rounded-xl border border-border shadow-sm relative overflow-hidden">
             <div className="absolute right-0 top-0 p-4 opacity-10"><Database size={64} /></div>
             <p className="text-xs font-bold text-muted-foreground uppercase">System Status</p>
             <h3 className="text-3xl font-bold text-green-600 mt-2 flex items-center gap-2"><CheckCircle2/> Healthy</h3>
             <p className="text-xs text-muted-foreground mt-4">DB Latency: 24ms</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
           <div className="bg-card border border-border rounded-xl shadow-sm p-5">
              <h3 className="font-bold text-foreground mb-4 text-sm uppercase">Platform Management</h3>
              <div className="grid grid-cols-2 gap-3">
                 <QuickActionBtn 
                    icon={<Building size={20}/>} label="Manage Companies" 
                    onClick={() => navigate('/companies')} 
                    colorClass="bg-blue-100 text-blue-600" 
                 />
                 <QuickActionBtn 
                    icon={<Database size={20}/>} label="Edit Constants" 
                    onClick={() => navigate('/constants')} 
                    colorClass="bg-purple-100 text-purple-600" 
                 />
                 <QuickActionBtn 
                    icon={<FileText size={20}/>} label="Master Syllabus" 
                    onClick={() => navigate('/tasks')} 
                    colorClass="bg-orange-100 text-orange-600" 
                 />
                 <QuickActionBtn 
                    icon={<Server size={20}/>} label="System Logs" 
           ...
           <div className="bg-card border border-border rounded-xl shadow-sm p-5 flex flex-col justify-center items-center text-center">
              <div className="p-4 bg-muted rounded-full mb-3"><Activity size={32} className="text-muted-foreground"/></div>
              <h3 className="font-bold text-foreground">Recent Signups</h3>
              <p className="text-sm text-muted-foreground mb-4">No new company signups in the last 24 hours.</p>
              <button className="text-xs text-primary font-bold hover:underline">View All Companies</button>
           </div>
        </div>
      </div>
    );
  ...
  // ==================================================================================
  // VIEW 2: SHORE ADMIN (FLEET MANAGER) DASHBOARD - (Original Code)
  // ==================================================================================
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      {/* HEADER */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card p-5 rounded-xl border border-border shadow-sm relative overflow-hidden ring-1 ring-orange-500/20">
           ...
           <button onClick={() => navigate('/approvals')} className="mt-3 w-full bg-orange-500 text-white text-xs font-bold py-1.5 rounded hover:bg-orange-600 transition-colors">
             Review Now
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
           ...
           <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex-1">
              <div className="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
                 <h3 className="font-bold text-foreground flex items-center gap-2"><CheckCircle2 size={18} className="text-orange-500" /> Pending Verification</h3>
                    ...
                    approvalQueue.map((item) => (
                       <div key={item.uniqueId} className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors group">
                          <div className="flex items-center gap-3">
                             <div className="w-8 h-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold">{item.cadetName.charAt(0)}</div>
                             <div>
                          ...
                          <button onClick={() => navigate('/approvals')} className="bg-background border border-border text-foreground px-3 py-1.5 rounded text-xs font-medium opacity-0 group-hover:opacity-100 transition-opacity hover:bg-primary hover:text-white hover:border-primary">Review</button>
                       </div>
                    ))
                 )}
              </div>
        ...
        <div className="space-y-6">
           ...
           <div className="bg-card border border-border rounded-xl shadow-sm overflow-hidden flex flex-col h-75">
              <div className="p-4 border-b border-border bg-muted/20 flex justify-between items-center">
                 <h3 className="font-bold text-foreground flex items-center gap-2"><Activity size={16} className="text-blue-500" /> Live Activity</h3>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                 {recentActivity.map((log) => (
                    <div key={log.id} className="flex gap-3 relative pb-4 last:pb-0 border-l border-border pl-4 last:border-0">
                       ...
                       <div>
                          <p className="text-xs text-foreground"><span className="font-bold">{log.user}</span> {log.action} <span className="font-medium text-primary">{log.target}</span></p>
                          <p className="text-[10px] text-muted-foreground mt-0.5">{log.time}</p>
                       </div>
                    </div>
                 ))}
              </div>
              <button onClick={() => navigate('/audit/main')} className="p-3 text-xs text-center border-t border-border text-muted-foreground hover:text-foreground hover:bg-muted transition-colors">View Full Audit Log</button>
           </div>
        </div>
      </div>
    </div>
