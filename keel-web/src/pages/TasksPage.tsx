...
const TasksPage: React.FC = () => {
  ...
  const getFunctionLabel = (funcId: string) => {
    // Strips FUNC-1 to 1 to find label in STCW_MAP
    const num = String(funcId || "").replace('FUNC-', '');
                      ...
                      .map((topic: any) => {
                        ...
                        return (
                          <div key={topic.id} className="border border-border rounded-lg overflow-hidden bg-background mb-4 shadow-sm animate-in fade-in slide-in-from-bottom-2">
                             ...
                             {/* ACCORDION HEADER - Shows Topic/Section Title */}
                             <button 
                             ...
                             >
                               <div className="flex items-center gap-3">
                                 ...
                                 <h3 className="font-bold text-foreground text-sm uppercase">{topic.title}</h3>
                                      ...
                                      <div key={task.id} className="group bg-background border border-border p-4 rounded-lg hover:border-primary/40 transition-all relative">
                                         ...
                                         <div className="flex justify-between items-start pr-20">
                                            <div className="space-y-1">
                                                <h4 className="font-bold text-sm text-foreground uppercase tracking-tight">{task.title}</h4>
                                                <p className="text-xs text-muted-foreground font-medium">{task.description}</p>
                                            </div>
                                         </div>
                                         
                                         <div className="flex flex-wrap gap-2 mt-3 items-center">
                                            {/* Matches stcw key from dataService */}
                                            <span className="text-[10px] font-mono bg-muted px-2 py-0.5 rounded text-muted-foreground border border-border">
                                               {task.stcw || 'A-II/1'}
                                            </span>
                                            {task.dept && <span className="text-[10px] border border-border px-2 py-0.5 rounded text-muted-foreground uppercase font-bold">{task.dept}</span>}
                                            {/* Matches safety key from dataService */}
                                            {task.safety && task.safety !== 'NONE' && (
                                               <span className="text-[10px] bg-red-500/10 text-red-600 px-2 py-0.5 rounded border border-red-500/20 flex items-center gap-1 font-bold">
